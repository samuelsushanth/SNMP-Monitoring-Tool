...
my $sql11="CREATE TABLE IF NOT EXISTS sam_assign1 (id int (11) NOT NULL AUTO_INCREMENT,IP tinytext NOT NULL,PORT int (11) NOT NULL,COMMUNITY tinytext NOT NULL, INTERFACES VARCHAR(200), INTERFACES_NAMES VARCHAR (5800), SYSTEM_NAME VARCHAR(4000), PRIMARY KEY ( id )) ENGINE=InnoDB DEFAULT CHARSET=latin1 AUTO_INCREMENT=1;";
...
print "$row[1]-$row[2]-$row[3]-@filtered_indices\n";
		...
		my $count=0;
							...
							if($counter_interfaces != 0) {
												...
												my $sthaa = $dbh->prepare("SELECT * FROM sam_assign1 WHERE IP='$row[1]' and PORT='$row[2]' and COMMUNITY='$row[3]'");
												$sthaa->execute();
												$COUNT= $sthaa->rows;

												if ($COUNT==0){		
												my $sqli="INSERT INTO sam_assign1(IP,PORT,COMMUNITY,INTERFACES,INTERFACES_NAMES,SYSTEM_NAME) values('$row[1]','$row[2]','$row[3]','@filtered_indices','@int_names','$aa')";
